...
const smallAnimalName=zooAnimals.map((value) => {return value.animal_name.toLowerCase()});
console.log(smallAnimalName);

   
  /* Request 3: .filter()
  The zoos are concerned about animals with a lower population count. Using filter, create a new array of objects called lowPopulationAnimals which contains only the animals with a population less than 5.
  */
 const lowPopulationAnimalss=zooAnimals.filter((value)=> value.population<5)
//        shine filterhsn utg    condition 
// const newArray = array.filter((value)=> )
console.log(lowPopulationAnimalss);

   
  /* Request 4: .reduce()
  The zoos need to know their total animal population across the United States. Find the total population from all the zoos using the .reduce() method. Remember the reduce method takes two arguments: a callback (which itself takes two args), and an initial value for the count.
  */
   const allPopulation=zooAnimals.reduce((total,value)=>{return total+value.population} , 0);
 ...
 console.log(consume(5,2,divide));
  ...
  */
  ...
  function createMenuItem(layyy) {
    const {name, price, category} = layyy
    const newMenu = {
        name: name,
        price: price,
        category: category,
  ...
  console.log(createMenuItem(breakfastBurrito));
//   console.log(burger);
  ...
  function calculatePriceBasedOnRole(role,lunch){
    let price;
    if(role==="teacher"|| role==="student"){
        price=lunch.price*0.8

    }else if(role==="public"){
        price=lunch.price*0.9
    }
    return price;
  }
  console.log(calculatePriceBasedOnRole("public",burger));
  ...
  function feedBacks(name){
    if(name.name==="Lauren"){
        
    }
  }
  console.log(feedBacks(reviews));
